        ...
        <div class="row d-flex justify-content-center p-4">
            <div class="dashbard-content-header d-flex align-items-center justify-content-between">
                <div class="title my-5">
                    <h1 class="text-capitalize p-0 fw-bold">Update post</h1>
                </div>
    ...
    <div class="breaking-news-form" style="margin-bottom: 50px;">
        <div class="container">
            <div class="forms mb-5 pb-5">
                <form action="{{Route('post_update',[$postshowid->id])}}" method="post" enctype="multipart/form-data">
                    @csrf
                    <div class="user-name mt-5">
                        <label for="" class="text-capitalize mb-2">post title</label>
                        <input class="form-control py-2" value="{{$postshowid->title}}" name="title" type="text" placeholder="post title" required>
                    </div>
                    <div class="select-any mt-5">
                        ...
                        <select type="text" class="selectpicker border w-100" name="categories[]" multiple required>
                            ...
                            <option 
                            @foreach ($postshowid->categories as $category)
                                {{$category->id == $parantName->id ? 'selected':''}}
                            @endforeach
                            value="{{$parantName->id}}" style="font-size: 1.9rem;">{{$parantName->name}}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="select-any mt-5">
                        ...
                        <select type="text" class="selectpicker border w-100" name="tags[]" multiple required>
                            ...
                            <option 
                            @foreach ($postshowid->tags as $tag)
                                {{$tag->id == $tagdata->id ? 'selected':''}}
                            @endforeach
                            value="{{$tagdata->id}}" style="font-size: 1.9rem;">{{$tagdata->name}}</option>
                            @endforeach
                        </select>
                    </div>
                     <div class="body mt-5">
